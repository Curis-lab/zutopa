//github.com/civitai/civitai/blob/ffb6062a9ca104907760ca7ebefd0390f7dcbdc6/src/utils/s3-client.ts#L72
//code for copying
...
function createS3Client({
  ...
  if (keys.length)
    throw new Error(`${name} S3Client: missing vars ${keys.join(", ")}`);
...
export const S3 = {
  uploadBucket: new S3Bucket({
    client: baseS3Client,
    bucket: env.S3_UPLOAD_BUCKET,
  }),
  settledBucket: new S3Bucket({
    client: baseS3Client,
    bucket: env.S3_SETTLED_BUCKET,
  }),
  imageBucket: new S3Bucket({
    client: baseS3Client,
    bucket: env.S3_IMAGE_UPLOAD_BUCKET,
  }),
  imageCacheBucket: new S3Bucket({
    client: baseS3Client,
