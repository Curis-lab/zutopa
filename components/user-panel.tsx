...
export function UserPanel({users}: props){
    ...
    return <div className="w-1/6 bg-gray-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-scroll py-4 flex flex-col gap-y-10">
            {users && users.map((user)=><UserCircle key={user.id} profile={user.profile} className="h-24 w-24 mx-auto flex-shrink-0" onClick={()=>{}//navigate(`/home/${user.id}`)
            }/>)}
        </div>
